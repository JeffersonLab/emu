...
//System.out.println("      DataChannel Et out: past barrier!");
    ...
    private class DataOutputHelper extends Thread {
        ...
        private Sequence etWriteSequence;
        ...
        DataOutputHelper(ThreadGroup group, String name) {
            ...
            // 1st consumer barrier of ring buffer
            etFillBarrier = rb.newBarrier();

            // 1st consumer sequence of ring buffer
            // which depends on (comes after) producer.
            etFillSequence = new Sequence(Sequencer.INITIAL_CURSOR_VALUE);

            // 2nd consumer - organizer of evio data to writers -
            // which depends on 1st consumer.
            SequenceBarrier etWriteBarrier = rb.newBarrier(etFillSequence);
            etWriteSequence = new Sequence(Sequencer.INITIAL_CURSOR_VALUE);

            // 3rd to Nth consumers - writers into ET buffers -
            // which all depend on 2nd consumer.
            SequenceBarrier etPutBarrier = rb.newBarrier(etWriteSequence);
            ...
            for (int i=0; i < writerThreadCount; i++) {
                writers[i] = new DataWriter(rb, etWriteSequence, etWriteBarrier, i);
                writers[i].start();
            }
         ...
         */
        synchronized public void writerRelease(long seq) {
            // If we got a new max ...
            if (seq > maxSequence) {
            ...
            if ( (maxSequence - lastSequenceReleased - 1L) == between) {
                etWriteSequence.set(maxSequence);
        ...
        final private class DataWriter extends Thread {
             ...
             */
            DataWriter(RingBuffer<EtContainer>rb, Sequence sequence,
                       SequenceBarrier barrier, int place) {
                this.rb = rb;
                this.place = place;
                this.barrier = barrier;
            ...
            public void run() {
                ...
                try {
                    ...
                    long nextSequence = sequence.get() + 1L + place;
...
//System.out.println("      DataChannel Et out: " + name + ":" + place + ", try getting next seq (" + nextSequence + ") for ET");
...
//                   ", items = " + container.itemCount);
                        ...
                        for (int i=0; i < container.itemCount; i++) {
                            ...
                            if (ringItemType == ModuleIoType.PayloadBank) {
                                writer.writeEvent(container.items[i].getEvent());
                            }
                            else {
                                EvioNode node  = container.items[i].getNode();
                                ByteBuffer buf = container.items[i].getBuffer();
                                if (buf != null) {
...
//System.out.println("      DataChannel Et out " + outputIndex + ": write node");
                                    writer.writeEvent(container.items[i].getNode(), false);
                                }
                            }
                            ...
                            container.items[i].releaseByteBuffer();
                        }
                        ...
                        //-----------------------------------------------------------------
                        if (container.isEnd) {
//System.out.println("      DataChannel Et out " + outputIndex + ": END seq = " +  (nextSequence) +
...
//System.out.println("      DataChannel Et out " + outputIndex + ": release ET event " + nextSequence);
                        writerRelease(nextSequence);
