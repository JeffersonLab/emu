 ...
 *
 * All build threads come after the producing thread in no particular order
 * and consume slots that the producer is finished with.
 ...
 * </code></pre><p>
 *
 * This class is the event building module and is a multi-threaded.
 *
 * Each BuildingThread - of which there may be any number - takes
 * one bank from each ring buffer (input channel), skipping every Mth,
 * and builds them into a single event.
 * But before building, it first does an examining of its Evio banks and sees
 * if they are in the proper format (ROC Raw, Physics, Control, User).
 * It throws an exception for any banks that are not in
 * the proper format and places any User events in the first output channel.
 *
 * The built event is placed in a ring buffer of
 ...
 */
public class FastEventBuilderNoPre extends ModuleAdapter {
     ...
     */
    public FastEventBuilderNoPre(String name, Map<String, String> attributeMap, Emu emu) {
        ...
        logger.info("  EB mod: using FastEventBuilderNoPre **************************");
...
System.out.println("  EB mod: create Build Thread with index " + btIndex + ", count = " + btCount);
        ...
        /**
         * This method is used to place
         * a bank onto the ring buffer of an output channel.
         *
         * @param outputChannelIndex  output channel identifier
         ...
         * @param bbSupply   ByteBuffer supply that item comes form
         * @throws InterruptedException
         */
        private void eventToOutputRing(int outputChannelIndex, ByteBuffer buf, EventType eventType,
            ...
            RingBuffer rb = outputChannels.get(outputChannelIndex).getRingBuffersOut()[btIndex];

//System.out.println("  EB mod: wait for out buf, ch" + outputChannelIndex + ", ring " + btIndex);
            long nextRingItem = rb.next();
//System.out.println("  EB mod: Got sequence " + nextRingItem + " for " + outputChannelIndex + ":" + btIndex);
...
//System.out.println("  EB mod: will publish to ring " + btIndex);
            rb.publish(nextRingItem);
//System.out.println("  EB mod: published to ring " + btIndex);
        }
       ...
       */
      private ControlType getAllControlEvents()
          ...
          for (int i=0; i < inputChannelCount; i++) {
              try  {
                  ControlType cType;
                  while (true) {
                      buildBarrierIn[i].waitFor(nextSequences[i]);
      ...
      /**
       * This method takes a ring item and makes sure each item is a valid, evio, DAQ event.
       ...
       * @param channel
       */
      private void preProcess(RingBuffer<RingItem> ringBuffer,  SequenceBarrier barrier,
...
System.out.println("  EB mod: bbSupply -> " + ringItemCount + " # of bufs, direct = " + false);
                ...
                try {
                    ...
                    ControlType cType = getAllControlEvents();
                    if (!cType.isPrestart()) {
...
System.out.println("  EB mod: got all GO events");
                ...
                while (moduleState == CODAState.ACTIVE || paused) {
                    ...
                    endEventCount = 0;
                    PayloadBuffer payloadBank=null;
                    //int printCounter = 0;
...
//System.out.println("  EB mod: bt" + btIndex + " ch" + i + ", available seq[" + i + "]  = " + availableSequences[i]);
                            ...
                            //--------------------------------------
                            // If dumping data for testing purposes
                            //--------------------------------------
                            if (dumpData) {
                                while (nextSequences[i] <= availableSequences[i]) {
                                    RingItem ri = ringBuffersIn[i].get(nextSequences[i]);
                                    ri.releaseByteBuffer();
                                    nextSequences[i]++;
                                }
                            ...
                            while (nextSequences[i] <= availableSequences[i]) {
                                payloadBank = buildingBanks[i] =
                                        (PayloadBuffer) ringBuffersIn[i].get(nextSequences[i]);
//System.out.println("  EB mod: bt" + btIndex + " ch" + i + ", event order = " + payloadBank.getByteOrder());
                                eventType = payloadBank.getEventType();

                                // Check the payload bank for correct format -
                                // physics, ROC raw, control, or user.
                                // All other buffers are ignored.
                                Evio.checkPayload(payloadBank, inputChannels.get(i));
                                ...
                                if (eventType.isUser()) {
                                    ...
                                    // Swap headers, NOT DATA, if necessary
                                    if (outputOrder != payloadBank.getByteOrder()) {
                                        try {
                                            //System.out.println("  EB mod: swap user event (not data)");
                                            ByteBuffer buffy = payloadBank.getBuffer();
                                            EvioNode nody = payloadBank.getNode();
                                            if (buffy != null) {
                                                // Takes care of swapping of event in its own separate buffer,
                                                // headers not data
                                                ByteDataTransformer.swapEvent(buffy, buffy, 0, 0, false, null);
                                                ...
                                                ByteDataTransformer.swapEvent(buffy, null, 0, 0, false, null);
                                                // Store in ringItem
                                                payloadBank.setBuffer(buffy);
                                                payloadBank.setNode(null);
                                                // Release claim on backing buffer since we are now
                                                // using a different buffer.
                                                payloadBank.releaseByteBuffer();
                                            }
                                        }
                                        catch (EvioException e) {/* should never happen */ }
                                    }
                                    ...
                                    eventToOutputChannel(payloadBank, 0, 0);
                                    //System.out.println("  EB mod: sent user event to output channel");
...
//System.out.println("  EB mod: bt" + btIndex + " ch" + i + ", accept item " + nextSequences[i]);
                            ...
                            if (!eventType.isControl()) {
                                ...
                                if (!gotFirstBuildEvent) {
                                    ...
                                    totalNumberEvents = payloadBank.getNode().getNum();
                            ...
                            if (!payloadBank.getControlType().isEnd()) {
                                throw new EmuException(payloadBank.getControlType() +
...
//if (debug) System.out.println("  EB mod: build physics event with ROC raw banks");
                    ...
                    eventToOutputRing(outputChannelIndex, builder.getBuffer(),
