...
public class EmuBlastee3 {
    ...
    /** Contains all data merging objects being used. */
    private ConcurrentHashMap<Integer, ClientMergerForEvio> mergers =
            new ConcurrentHashMap<>();
    ...
    // Ring buffer stuff
    private long nextSequence, availableSequence = -1L;
    private Sequence sequence;
    private SequenceBarrier barrier;
    private RingBuffer<RingItem> ringBuffer;
    boolean isER = false;
    ...
    /** Method to print out correct program command line usage. */
     ...
     public static void main(String[] args) {
         try {
             EmuBlastee3 blastee = new EmuBlastee3(args);
             blastee.run();
         }
         catch (Exception e) {
    ...
    public void run() {
        ...
        if (!parseData) {
            mergerForBuffersThread = new ClientMergerForBuffers();
             ...
             // Create a ring buffer. The ByteBuffers get
             // parsed into evio events & placed into it.
...
System.out.println("Got connection from " + id + ", socket count = " + socketCount +
                   ", pos = " + socketPosition);

                if (parseData) {
                    ClientMergerForEvio merger = mergers.get(id);

                    if (merger == null) {
                        // Create obj/thd to handle client connection merging thread
...
System.out.println("Client " + me + ", bufferSize = " + bufferSize);
...
//            }
//            else {
//                System.out.println("Client " + me + ", parseData = false");
//            }
...
//System.out.println("Publish BB " + me);
     ...
     */
    private final void parseToRing(ByteBufferItem item, ByteBufferSupply bbSupply) throws IOException, EvioException {
...
//System.out.println("rb.next()");
            ...
            // The last event parsed from the buffer means the reader should
            // switch to the next ring.
            if (i == eventCount) {
...
System.out.println("EmuBlastee: found END event");
    ...
    /** Class to consume all evio ring items and dump them. */
    private class RingDumper extends Thread {
...
//System.out.println("merger: waitfor, availSeq = " + availableSequence + ", nextSeq = " +
//nextSequence + ", barrier = " + barrier);
                        availableSequence = barrier.waitFor(nextSequence);
                    }

//System.out.println("merger: get ");
                    RingItem ri = ringBuffer.get(nextSequence);
    ...
    private class ClientMergerForEvio extends Thread {
        ...
        int socketCount;
        int handlerCount;
        /** Contains all data merging objects being used. Value not used. */
        ClientHandler[] handlers;
        /** Contains all byte buffer supply objects being used. */
        ByteBufferSupply[] supplies;

        /** Thread used to clear evio data ring. Used when parsing. */
        private RingDumper ringDumper;


        ClientMergerForEvio(int id, int socketCount)  {
            this.id = id;
            this.socketCount = socketCount;
            handlers = new ClientHandler[socketCount];
            supplies = new ByteBufferSupply[socketCount];
            // Create and start evio event dumper thread
            ringDumper = new RingDumper();
        }


        public void addHandler(ClientHandler handler) {
            // Should never happen, but ...
            if (handlerCount >= socketCount) {
            ...
            handlers[handler.socketPosition - 1] = handler;
            supplies[handler.socketPosition - 1] = handler.bbSupply;
            handlerCount++;
        }


        public boolean isReady() {
            return (handlerCount == socketCount);
        }
        ...
        public void startHandlers() {
            for (ClientHandler client : handlers) {
                if (client != null) {
                    client.start();
System.out.println("Started client " + client.me);
                }
            }
        }
...
System.out.println("Starting merger");
            ...
            try {
                while (true) {
                    for (ByteBufferSupply bbSupply : supplies) {
