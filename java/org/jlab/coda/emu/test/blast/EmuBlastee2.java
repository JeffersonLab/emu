...
System.out.println("Merger is ready (handler count = " + merger.handlerCount + "), start merger");
    ...
    private class ClientHandler extends Thread {
        ...
        ClientHandler(int id, Socket socket, DataInputStream in, int socketCount, int socketPosition) {
            ...
            if (parseData) {
                // Create another ring buffer. The ByteBuffers get
...
//System.out.println("Publish BB " + me);
            ...
            if (parseData) {
                ...
                mergerForEvioThread = new ClientMergerForEvio(id, socketCount);
            }
            else {
...
//System.out.println("merger: waitfor, availSeq = " + client.availableSequence + ", nextSeq = " +
//client.nextSequence + ", barrier = " + client.barrier);
...
//System.out.println("merger: get ");
                            ...
                            if (pCounter++ < 565) {
                                // Data does NOT include bank header
                                long[] data = ri.getNode().getLongData();
                                System.out.println(client.socketPosition + " ts = " + data[2] +
                                                           ", 0x" + Long.toHexString(data[2]));
                                //Utilities.printBuffer(ri.getNode().getStructureBuffer(true), 0, 40, "event");
                            }
