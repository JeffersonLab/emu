...
System.out.println("    DataTransportImplCMsg.callback : got msg of (sub,typ) " + msg.getSubject() + ", " + msg.getType());

        if (type.matches("get_socket")) {
            try {
System.out.println("    DataTransportImplCMsg.callback : myinstance = " + myInstance);
    ...
    public class AcceptHelper extends Thread {
        ...
        /** Method run ... */
        public void run() {
System.out.println("    DataTransportImplCMsg.AcceptHelper.run : for " + this.getName());
...
//System.out.println("Invalid connection, got " + magic + " expected " + 0xC0DA2008);
                    ...
                    // length of name
                    int l = in.readByte();
System.out.println("    DataTransportImplCMsg.AcceptHelper.run : name length = " + l);

                    // name
                    byte[] cbuf = new byte[l];
                    in.readFully(cbuf, 0, l);
                    String cname = new String(cbuf, 0, l, "US-ASCII");
System.out.println("    DataTransportImplCMsg.AcceptHelper.run : channel name = " + cname);

                    // retrieve already-created channel for connecting client
                    DataChannelImplCMsg c = (DataChannelImplCMsg) channels().get(cname);
System.out.println("    DataTransportImplCMsg.AcceptHelper.run : retrieved channel object = " + c);
//System.out.println("    DataTransportImplCMsg.AcceptHelper.run : channels " + channels() + " " + DataTransportImplCMsg.this);
                    if (c != null) {
...
System.out.println("    DataTransportImplCMsg.AcceptHelper.run : end");
...
System.out.println("    DataTransportImplCMsg.startServer : IN");
...
System.out.println("    DataTransportImplCMsg.startServer : subscribe to (sub,typ) " + name() + ", get_socket");
    ...
    private void connect() throws DataTransportException {
        try {
            ...
            //msg.setText("no text");
System.out.println("    DataTransportImplCMsg.connect : sendAndGet msg to (sub,typ) " + name() + ", get_socket");
...
//System.out.println("    DataTransportImplCMsg.createChannel : channels " + channels() + " " + this);
        return c;
    }
...
System.out.println("    DataTransportImplCMsg.execute : " + cmd);
        ...
        if (cmd.equals(CODATransition.PRESTART)) {
            ...
            try {
                if (server) {
                    System.out.println("    DataTransportImplCMsg.execute PRE : call startServer for " + name());
                    startServer();
                }
                else {
                    System.out.println("    DataTransportImplCMsg.execute PRE : call connect to " + name());
                    connect();
                }
            ...
            try {
                Logger.debug("    DataTransportImplCMsg.execute END/RESET: cmsg unsubscribe : " + name() + " " + myInstance);
