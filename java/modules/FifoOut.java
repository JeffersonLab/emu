 ...
 * <p/>
 */
public class FifoOut implements EmuModule, Runnable {
...
System.out.println("Process module: quitting watcher thread");
     ...
     */
    public FifoOut(String name, Map<String, String> attributeMap, Emu emu) {
        this.emu = emu;
        this.name = name;
...
System.out.println("FifoOut: is active or paused");
            try {
                ...
                while (state == CODAState.ACTIVE) {
System.out.println("FifoOut: is while loop");

                    if (hasOutputs && !outputIter.hasNext()) {
System.out.println("FifoOut: Has output channels");
                    ...
                    //add a bank of ints
                    int[] data = {counter++};
                    EvioBank bank1 = new EvioBank(111, DataType.INT32, 0);
                    try {
                        eventBuilder.appendIntData(bank1, data);
                        eventBuilder.addChild(event, bank1);
                    }
                    ...
                    if (hasOutputs) {
System.out.println("FifoOut: put bank on output Q");
                        out_queue.put(event);
                    }
                }
...
System.out.println("GO in FifoOut module");
