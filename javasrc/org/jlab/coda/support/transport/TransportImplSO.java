 ...
 */
public class TransportImplSO extends DataTransportCore implements DataTransport {
     ...
     */
    public TransportImplSO(String pname, Map<String, String> attrib) throws DataNotFoundException {
        super(pname, attrib);
    }
    ...
    public void startServer() throws TransportException {
        ...
        try {
            ...
            e1.printStackTrace();
            throw new TransportException(e1.getMessage());
        }
        ...
        Thread multicastHelperThread = new Thread(EMUComponentImpl.THREAD_GROUP, new MulticastHelper(), name() + " multicast");
    ...
    private class ConnectHelper implements Runnable {
        ...
        public void run() {
            try {
                ...
                while (true) {
                    String s = name();
                    DatagramPacket odp = new DatagramPacket(s.getBytes(), s
                            .length(), multicastAddr, multicastPort);
                    ...
                    if (values[0].matches(name())) {
                        ...
                        Socket dataSocket = new Socket(values[1], Integer
                                .parseInt(values[2]));
                        ...
                        String emuName = EMUComponentImpl.INSTANCE.name();
                        String channelName = name() + ":" + emuName;

                        dout.write(channelName.length());

                        dout.write(channelName.getBytes());
                        DataChannelImplSO c = (DataChannelImplSO) channels
                                .get(name() + ":" + emuName);
    ...
    /** Method connect ... */
    public void connect() {
        Thread connectHelperThread = new Thread(EMUComponentImpl.THREAD_GROUP, new ConnectHelper(), name() + " connect");
