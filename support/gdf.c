...
gdf_new(char *name)
{
    ...
    cbp = (gdf_struc *) malloc(sizeof (gdf_struc));
    bzero((void *) cbp,sizeof(gdf_struc));
    cbp->data = (void **) malloc(QSIZE*sizeof(void*));
    bzero((void *) cbp->data,QSIZE*sizeof(void*));
    cbp->qsize = QSIZE;
...
gdf_struc *
gdf_new_sized(char *name,int qsize)
{
    gdf_struc *cbp;
 ...
 */
int
put_cb_data(gdf_struc *cbh, void *data)
{
    gdf_struc *cbp = cbh;
    int st;
    ...
    while ((cbp->num_full == cbp->qsize) && (cbp->deleting == 0))
    {
...
gdf_get(gdf_struc *cbh)
{
    ...
    while (cbp->num_full == 0)
    {
        ...
        status = pthread_cond_timedwait(&cbp->notempty, &cbp->buf_lock,&ts);
