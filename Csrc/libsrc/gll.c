...
void gll_delete_li(gll_li lp) {
	if (lp == NULL) return;

	gll_el le = gll_find_el(gll_li_li,lp);
...
gll_el gll_add_el (gll_li lp, void * data) {
    ...
    if (lp->last != NULL) {
        // if last is not NULL then we family on to end of list
        le->previous = lp->last;
        le -> next = NULL;
...
gll_el gll_remove_el (gll_el el) {
    ...
    if (le == NULL) {
        printf("WARNING: searching for %px when list %s was empty!\n", el, lp->name);
        pthread_mutex_unlock(&lp->lock);
        return NULL;
    }
    ...
    le = NULL;
    printf("WARNING: %p not found in list %s\n", el,lp->name);
done_remove:
    pthread_mutex_unlock(&lp->lock);
    return le;
}
...
gll_el gll_find_el (gll_li li,void *data) {
    ...
    if (le == NULL) {
        printf("WARNING: searching for %p when list %s was empty!\n", le, li->name);
